class PdfsController < ApplicationController
  ...
  def show
    ...
    respond_to do |format|
      format.html
      format.pdf do
        pdf = AnalyticsPdf.new(@pdf)
        send_data pdf.render, filename: "pdf_#{@pdf.business_name}.pdf",
                              type: "application/pdf",
                              disposition: "inline"
      end
    end
    @bot_user = Bot_user.all
  end
  ...
  private
    ...
    def pdf_params
      params.require(:pdf).permit(:amount_of_users, :business_name, :date)
    end
end
